import {Card , CardContent , CardActions , Button , Typography , CardMedia} from '@material-ui/core';
import ThumbUpAltIcon from '@material-ui/icons/ThumbUpAlt';
import DeleteIcon from '@material-ui/icons/Delete';
import MoreHorizIcon from '@material-ui/icons/MoreHoriz';
import moment from 'moment';
import useStyles from './Styles'
import { useContext } from 'react';
import { MemoriesContext } from '../../../contextApi/MemoriesContext';
import {deletePost , likePost} from '../../../apis/index'
const Post = ({post}) => {
    const classes = useStyles();
    const {dispatch} = useContext(MemoriesContext);
    // Delete Post 
    const DeletePost = async(id) => {
        try {
            await deletePost(id);
            dispatch({type:"DELETE_POST" , payload:id});
        } catch (error) {
            console.log(error.message)
        }
    }
    // Like Post 
    const LikePost = async (id) => {
        try {
            const newPosts = await likePost(id);
            dispatch({type:"LIKE_POST" , payload:newPosts})
        } catch (error) {
            console.log(error.message);
        }
    }
    return(
        <Card className={classes.card}>
            <CardMedia className={classes.media} image={post.selectedFile} title={post.title} />
            <div className={classes.overlay}>
                <Typography variant="h6">{post.creator}</Typography>
                <Typography variant='body2'>{moment(post.createdAt).fromNow()}</Typography>
            </div>
            <div className={classes.overlay2}>
                <Button style={{ color: 'white' }} size="small" onClick={() => dispatch({type:"SET_CURRENT_ID" , payload:post._id})}><MoreHorizIcon fontSize="default" /></Button>
            </div>
            <div className={classes.details}>
                <Typography variant="body2" color="textSecondary" component="h2">{post.tags.map((tag) => `#${tag} `)}</Typography>
            </div>
            <Typography className={classes.title} gutterBottom variant="h5" component="h2">{post.title}</Typography>
            <CardContent>
                <Typography variant="body2" color="textSecondary" component="p">{post.message}</Typography>
            </CardContent>
            <CardActions className={classes.cardActions}>
                <Button size="small" color="primary" onClick={() => {LikePost(post._id)}}><ThumbUpAltIcon fontSize="small" /> &nbsp; Like &nbsp;{post.likeCount} </Button>
                <Button size="small" color="primary" onClick={() => {DeletePost(post._id)}}><DeleteIcon fontSize="small" /> &nbsp; Delete &nbsp;</Button>
            </CardActions>
        </Card>
    )
}
export default Post;